...
export default function AppsPage() {
  ...
  const [apps, setApps] = useState([]);  // 👈 SIN <any[]>
  ...
  const getStatusIcon = (status) => {  // 👈 SIN TIPOS
    switch (status) {
      case 'ready':
        ...
        {filteredApps.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg p-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              No hay apps para mostrar
            </h3>
            <p className="text-gray-600 mb-6">
              {search || filter !== 'all' 
                ? 'Intenta con otros filtros de búsqueda'
                : 'Comienza creando tu primera app'}
            </p>
            {!search && filter === 'all' && (
              <Link
                href="/create"
                className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all inline-flex items-center gap-2"
              >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredApps.map((app) => (
              <Link
                key={app.id}
                href={`/apps/${app.id}`}
                className="group bg-white rounded-xl shadow-lg border-2 border-gray-100 p-6 hover:border-blue-400 hover:shadow-xl transition-all cursor-pointer"
              >
                ...
                {app.deploy_url && (
                  <div className="mt-3 flex items-center gap-1 text-xs text-green-600">
                    ...
                    <span className="truncate">Desplegada</span>
                  </div>
                )}
              </Link>
            ))}
